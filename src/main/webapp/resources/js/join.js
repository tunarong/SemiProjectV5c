...
function checkuid() {
    $.ajax({
        url:'/join/checkuid', type:'GET', data:{uid:$('#uid').val()}
    }) // 비동기 요청
     .done(function (data) {
        // alert(data);
         var msg = "사용 불가능한 아이디 입니다."
         if (data.trim() == '0') {msg = '사용 가능한 아이디입니다.';}
         $('#uidmsg').text(msg);
     }) // 처리 완료
...
function findzipcode() {
    $.ajax({
        url:'/join/zipcode', type:'GET', data:{dong:$('#dong').val()}
    }) // 비동기 요청 정의
     .done(function (data) {
        // 서버에서 넘어온 데이터는 JSON형이므로 출력시 Object로만 보여진다.
        // alert("데이터 : "+$('#dong').val()+"\n"+"결과 : "+data);
        var opts = "";
